...
class Team extends Controller
{
    ...
    public function reentryAccount(Request $request)
    {
      $user=Auth::user();
        $limit = $request->limit ? $request->limit : 10;
        $status = $request->status ? $request->status : null;
    ...
    $this->data['direct_team'] =$notes;
    $this->data['search'] =$search;
    $this->data['page'] = 'user.team.my-reEntry';
    return $this->dashboard_layout();
          ...
          $notes = User::where(function($query) use($ids)
{
  if(!empty($ids)){
    ...
    public function AutoPoolBonus(Request $request)
    {
      $user=Auth::user();
        $limit = $request->limit ? $request->limit : 10;
        $status = $request->status ? $request->status : null;
    ...
    $this->data['direct_team'] =$notes;
    $this->data['search'] =$search;
    $this->data['page'] = 'user.team.AutoPoolBonus';
    return $this->dashboard_layout();
